...
export class ItemJson {
    ...
    static getItemJson(itemStack) {
        ...
        if (itemStack.keepOnDeath)
            data.keepOnDeath = true;
        if (itemStack.lockMode !== ItemLockMode.none)
            data.lockMode = itemStack.lockMode;
        if (itemStack.nameTag)
            data.nameTag = itemStack.nameTag;
        if (lore)
            data.lore = lore;
        ...
        if (potion?.potionEffectType.id) {
            ...
            data.amount = 1;
            data.effectId = potion.potionEffectType.id;
            data.deliveryType = potion.potionDeliveryType.id;
        }
        else {
            ...
            if (canPlaceOn.length)
                data.canPlaceOn = canPlaceOn;
            if (durability && durability.damage)
                data.durability_damage = durability.damage;
    ...
    static getItemStack(itemJson) {
        const itemStack = !itemJson.hasOwnProperty("effectId") ? new ItemStack(itemJson.typeId, itemJson.amount) : Potions.resolve(itemJson.effectId, itemJson.deliveryType);
        if (itemJson.keepOnDeath)
            itemStack.keepOnDeath = true;
        if (itemJson.lockMode)
            itemStack.lockMode = itemJson.lockMode;
        if (itemJson.nameTag)
            itemStack.nameTag = itemJson.nameTag;
        if (itemJson.lore?.length)
        ...
        if (!itemJson.hasOwnProperty("effectId")) {
            ...
            if (itemJson.durability_damage) {
                const durability = itemStack.getComponent(ItemDurabilityComponent.componentId);
                if (durability)
                    durability.damage = itemJson.durability_damage;
            }
            ...
            if (itemJson.container) {
                const container = itemStack.getComponent(ItemInventoryComponent.componentId)?.container;
                if (container)
                    for (const key in itemJson.container) {
    ...
    static canBeStacked(itemJson, withItemJson) {
        if (itemJson.typeId !== withItemJson.typeId)
            return false;
        const maxAmount = this.getMaxAmount(itemJson.typeId);
        if (!maxAmount || maxAmount <= (itemJson.amount || 1))
            return false;
        if ((itemJson.amount ?? 1) + (withItemJson.amount ?? 1) > maxAmount)
            return false;
        if (itemJson.nameTag !== withItemJson.nameTag)
            return false;
        if (JSON.stringify(itemJson.lore) !== JSON.stringify(withItemJson.lore))
            return false;
        if (JSON.stringify(itemJson.canDestroy) !== JSON.stringify(withItemJson.canDestroy))
            return false;
        if (JSON.stringify(itemJson.canDestroy) !== JSON.stringify(withItemJson.canDestroy))
            return false;
        if (JSON.stringify(itemJson.canPlaceOn) !== JSON.stringify(withItemJson.canPlaceOn))
            return false;
        if (JSON.stringify(itemJson.container) !== JSON.stringify(withItemJson.container))
            return false;
        if ((itemJson.keepOnDeath ?? false) !== (withItemJson.keepOnDeath ?? false))
            return false;
        if ((itemJson.lockMode ?? ItemLockMode.none) !== (withItemJson.lockMode ?? ItemLockMode.none))
            return false;
        return true;
    }
    /**Add item data to array inventory */
    static addItemToArray(itemJson, items) {
        const maxAmount = this.getMaxAmount(itemJson.typeId);
        for (let i = 0; i < items.length; i++) {
            const withItemJson = items[i];
            if (itemJson.typeId !== withItemJson.typeId)
                continue;
            if (!maxAmount || maxAmount <= (withItemJson.amount || 1))
                continue;
            if (itemJson.nameTag !== withItemJson.nameTag)
                continue;
            if (JSON.stringify(itemJson.lore) !== JSON.stringify(withItemJson.lore))
