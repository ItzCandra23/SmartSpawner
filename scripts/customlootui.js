...
export class CustomLootUI {
    static async askItemLoot(player, itemBase, message) {
        ...
        try {
            ...
            if (chance !== chance)
                return this.askItemLoot(player, itemBase, "§cInvalid chance!");
            if (chance > 100 || chance < 0)
                return this.askItemLoot(player, itemBase, "§cChance out of range!");
            const min_amount = Number(res.formValues[2]);
            if (min_amount !== min_amount)
                return this.askItemLoot(player, itemBase, "§cInvalid minimum amount!");
            if (min_amount > (itemBase.amount ?? 1) || min_amount < 0)
                return this.askItemLoot(player, itemBase, "§cMinimum Amount out of range!");
            return {
                chance,
