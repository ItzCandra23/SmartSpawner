...
export interface ActiveSpawnerData {
    entityId: string;
    location: Vector3;
    dimensionId: string;
    stack: number;
...
export class ActiveSpawner {
     ...
     */
    static getActiveSpawnerKeys(): string[] {
        return world.getDynamicPropertyIds().filter((v) => v.startsWith("SmartSpawnerBlock:"));
    }

    /**Get all active spawner data */
    static getAllActiveSpawner(): ActiveSpawnerData[] {
        return this.getActiveSpawnerKeys().map((key) => JSON.parse(world.getDynamicProperty(key) as string));
    }

    /**Get active spawner data */
    static getActiveSpawner(location: Vector3, dimensionId: string): ActiveSpawnerData | undefined {
     ...
     */
    static setActiveSpawner(entityType: EntityType, block: Block): [string, ActiveSpawnerData] {
        if (block.typeId !== "minecraft:mob_spawner") throw new Error("Invalid block type!");
        if (this.getActiveSpawner(block.location, block.dimension.id)) throw new Error("Already set as Active Spawner");
    ...
    static deleteActiveSpawner(location: Vector3, dimensionId: string): void {
    ...
    */
    ...
    static addExperienceLoot(experience: number, location: Vector3, dimensionId: string): void {
        ...
        if ((data.expecience + experience) > max_experience) data.expecience = max_experience;
        else data.expecience += experience;
    ...
    static* generateSpawnerLoot(location: Vector3, dimensionId: string): Generator<void, void, void> {
        ...
        for (let i = 0; i < amount; i++) {
            let lootResult: { type: "itemJson"; items: ItemBase[]; } | { type: "itemStack"; items: ItemStack[]; } | undefined = undefined;
            ...
            if (!lootResult && configuration.loot.generateFromTable) {
                ...
                for (const pathTable of pathTables) {
                    ...
                    if (lootItems) lootResult.items.push(...lootItems);
                }
            }

            if (!lootResult && !configuration.loot.generateFromTable) {
                const lootItems = lootTableManager.generateLootFromEntityType(entityType);
                if (lootItems) lootResult = {
                    type: "itemStack",
                    items: lootItems,
                }
            }

            if (!lootResult?.items.length) continue;

            for (const itemLoot of lootResult.items) {
                try {
                    const itemJson = lootResult.type === "itemStack" ? ItemJson.getItemJson(itemLoot as ItemStack) : itemLoot;
    ...
    if (itemStack && itemStack.typeId === "minecraft:mob_spawner") {
        ...
        if (activespawner) {
            ...
            system.run(() => {
                ...
                if (entityType.id !== activespawner.entityId) return;
                ...
                try {
                    ...
                    } else container.setItem(selectedSlotIndex);
...
world.beforeEvents.explosion.subscribe((ev) => {
    ...
    ev.getImpactedBlocks().forEach((block) => {
        if (!ActiveSpawner.getActiveSpawner(block.location, block.dimension.id)) impactedBlocks.push(block);
    });
