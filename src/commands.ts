...
system.beforeEvents.startup.subscribe(({ customCommandRegistry }) => {
    ...
    }, (o, entityType: EntityType, amount: number, players?: Player[]) => {
        const targets = o.sourceEntity instanceof Player ? [o.sourceEntity] : players;

        if (!targets || !targets.length) return { status: CustomCommandStatus.Failure, message: "Player not found!" };
    ...
    customCommandRegistry.registerCommand({
    ...
    }, (o, action?: LootAction, entity?: EntityType, select?: SelectLoot, value?: string) => {
        const player = o.sourceEntity instanceof Player ? o.sourceEntity : undefined;

        function getSelectedXP(): [number, number] | undefined {
            if (select !== SelectLoot.xp) return undefined;
            if (!value) return undefined;
            
            const rawvalue = value.split("-");
            if (!rawvalue.length) return undefined;

            let xp1 = Number(rawvalue[0]);
            if (xp1 !== xp1 || xp1 < 0) return undefined;
            
            xp1 = Math.floor(xp1);

            if (rawvalue.length === 1) return [xp1, xp1];
            
            let xp2 = Number(rawvalue[1]);
            if (xp2 !== xp2 || xp2 < xp1) return undefined;
        ...
        function getSelectedLoot(): ItemStack[] | string | undefined {
            if (!select) return undefined;
            if (select === SelectLoot.table) return value?.trim();
            ...
            if (select === SelectLoot.hotbar) {
                ...
                for (let i = 0; i < 9; i++) {
                    ...
                    if (itemStack) items.push(itemStack);
                }
        ...
        if (!action) {
            ...
            const list = data.map((loot) => {
                ...
                if (loot.experience[0] !== 0 && loot.experience[1] !== 0) {
                    if (loot.experience[0] === loot.experience[1]) res += ` [${loot.experience[0]} XP]`;
                    else res += ` [${loot.experience[0]}-${loot.experience[1]} XP]`;
                }

                if (loot.loot.length) res += ` [${loot.loot.length} Items]`;
                if (loot.lootTables.length) res += ` [${loot.lootTables.length} Tables]`;
        ...
        if (action === LootAction.create) {
            if (CustomLoot.getLoot(entity.id)) return {
                status: CustomCommandStatus.Failure,
                message: "Loot Already!",
            };

            const experience = (typeof MobExperiences[entity.id] !== "object" ? [MobExperiences[entity.id] ?? 0, MobExperiences[entity.id] ?? 0] : MobExperiences[entity.id]) as [number, number];
            ...
            if (typeof rawloot === "object") {
                system.run(async () => {
                    const loot = await Promise.all(rawloot.map(async (v) => {
                        ...
                        if (!player) return ({ chance: 100, min_amount: itemJson.amount ?? 1, item: itemJson });

                        const itemLoot = await CustomLootUI.askItemLoot(player, itemJson);
                        if (!itemLoot) return ({ chance: 100, min_amount: itemJson.amount ?? 1, item: itemJson });
        ...
        if (action === LootAction.add) {
            ...
            if (typeof rawloot === "object") {
                system.run(async () => {
                    ...
                    for (const loot of loots) CustomLoot.addLootItem(entity.id, loot);
